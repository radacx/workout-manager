...
{
  internal abstract class ObservableRangeCollection<T> : ObservableCollection<T>
  {
    //------------------------------------------------------
    //
    //  Private Fields
    ...
    public void RemoveRange(IEnumerable<T> collection)
    {
      ...
      if (Count == 0)
        ...
        foreach (KeyValuePair<int, List<T>> cluster in clusters)
          OnCollectionChanged(
            new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Remove, cluster.Value, cluster.Key)
          );
      ...
      using (BlockReentrancy())
        using (DeferEvents())
        {
          for (var i = 0; i < count; i++, index++)
          {
            ...
            {
              OnCollectionChanged(
                new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Remove, cluster, clusterIndex)
              );
    ...
    public void Replace(T originalItem, T newItem)
    {
      using (BlockReentrancy())
      {
        using (DeferEvents())
        {
          ...
          {
            if (!ReferenceEquals(Items[i], originalItem))
            {
              continue;
            }
